...
class CMSCrawler:
    ...
    def parse_file_as_dataframe(file_path, group_name, asset_name):
        """Parse and filter vibration data from the file."""
            ...
            if data_section:
                ...
                for line in data_section.strip().split('\n'):
                    ...
                    if len(parts) >= 2:
                        ...
                        rows.append([float(timestamp), float(value), component])  # Include the component in the data
...
def main(start, end):
    ...
    for system_name, devices in systems_data.items():
        for device_id, device_name in devices:
            ...
            # Fetch raw files data for the device
...
if __name__ == "__main__":
    ...
    combined_df.to_csv("vibration_dataframes.csv", index=False)
    print("Filtered vibration data has been saved to 'vibration_dataframes.csv'")